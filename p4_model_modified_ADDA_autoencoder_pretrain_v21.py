...
from utils import MyDataset, trainTestSplit

from tqdm import tqdm, trange
from codes_before.data_preprocess import h5adTodf
from codes_before.eval_result import output_eval
from utils import preprocess, ProcessInputData
from evaluation import CCCscore
...
def pre_train(num_epochs, lr, save_step, autoencoder, train_loader, model_path):
    ...
    # ## Save the parameters training by pre-train model
    # if not os.path.exists(model_path):
    #     os.makedirs(model_path)
    # # torch.save(autoencoder, model_path+'/autoencoder.pth')
    # torch.save(autoencoder.state_dict(), model_path+'/autoencoder_params.pth')
...
def train_adda(num_epochs, lr, save_step, autoencoder, discriminator, src_data_loaders, tgt_data_loader, alpha_predict=1,alpha_DA=1, ip_dim=1, k=4):
    ''' ADDA Training using symmetric mapping '''

    ###------ (1) Define loss function ------###
    ### Define loss for predictor ###
    criterion_predict = nn.L1Loss()
    # criterion_predict = F.l1_loss()
    ...
    # optimizer_decoder = torch.optim.Adam(decoder_parameters, lr=lr, betas = (0.5, 0.8)) # , betas = (0.5, 0.9)
    # optimizer_encoder = torch.optim.Adam(encoder_parameters, lr=lr, betas = (0.5, 0.8)) # , betas = (0.5, 0.9)
    # optimizer_predicter = torch.optim.Adam(predicter_parameters, lr=lr, betas = (0.5, 0.8)) # , betas = (0.5, 0.9)
    #
    # optimizer_encoder_dis = torch.optim.Adam(encoder_parameters, lr=lr, betas = (0.5, 0.8)) # , betas = (0.5, 0.9)
    # optimizer_discriminator = torch.optim.Adam(discriminator.parameters(), lr=lr, betas = (0.5, 0.8))

    optimizer_decoder = torch.optim.AdamW(decoder_parameters, lr=lr) # , betas = (0.5, 0.8), weight_decay=1e-2
    optimizer_encoder = torch.optim.AdamW(encoder_parameters, lr=lr) # , betas = (0.5, 0.9)
    optimizer_predicter = torch.optim.AdamW(predicter_parameters, lr=lr) # , betas = (0.5, 0.9)

    # optimizer_encoder_dis = torch.optim.AdamW(encoder_parameters, lr=lr, betas = (0.5, 0.8), weight_decay=1e-2) # , betas = (0.5, 0.9)
    optimizer_discriminator = torch.optim.AdamW(discriminator.parameters(), lr=lr)
    optimizer_encoder_dis = torch.optim.AdamW(encoder_parameters, lr=lr) # , betas = (0.5, 0.9)
    ...
    for epoch in tqdm(range(num_epochs), position=0):
        for step, ((src_x, src_y), (tgt_x, tgt_y)) in enumerate(zip(src_data_loaders['train'], tgt_data_loader)):
            ##########################################################################
...
def train_adda1(num_epochs, lr, save_step, autoencoder, discriminator, src_data_loaders, tgt_data_loader, alpha_predict=1,alpha_DA=1, ip_dim=1, k=4):
    ''' ADDA Training using symmetric mapping '''
...
def train_adda2(num_epochs, lr, save_step, autoencoder, discriminator, src_data_loaders, tgt_data_loader, alpha_predict=1, alpha_DA=1, ip_dim=1, k=4):
    ''' ADDA Training using symmetric mapping '''
    ...
    # optimizer_decoder = torch.optim.AdamW(decoder_parameters, lr=lr, betas = (0.5, 0.8), weight_decay=1e-2) # , betas = (0.5, 0.8), weight_decay=1e-2
    # optimizer_encoder = torch.optim.AdamW(encoder_parameters, lr=lr, betas = (0.5, 0.8), weight_decay=1e-2) # , betas = (0.5, 0.9)
    # optimizer_predicter = torch.optim.AdamW(predicter_parameters, lr=lr, betas = (0.5, 0.8), weight_decay=1e-2) # , betas = (0.5, 0.9)
    # optimizer_discriminator = torch.optim.AdamW(discriminator.parameters(), lr=lr, betas = (0.5, 0.8), weight_decay=1e-2)
    ...
    for epoch in tqdm(range(num_epochs), position=0):
        for step, ((src_x, src_y), (tgt_x, tgt_y)) in enumerate(zip(src_data_loaders['train'], tgt_data_loader)):
            ...
            total_loss = alpha_predict * predict_loss_src + alpha_recon * recon_loss
            total_loss.backward()
...
def train_model(pre_dt, pre_prop, source_dt, source_prop, target_dt, target_prop,
                batch_size=128, seed=0, run=1, epoch_pre =200, epoch_fine=200, lr_pre = 1e-4, lr_train = 1e-4, model_path='/model', data_name='pancreas', run_now='baron'):
    ...
    model_path_final = model_path + '/' + '/autoencoder_params.pth'
    flag = os.path.exists(model_path_final)

    # if flag:
    #     print("Pre-trained model was already existing ...")
    #     autoencoder_me.load_state_dict(torch.load(model_path + '/autoencoder_params.pth'))
    # else:
    #     print("Pre-trained model training is starting ...")
    #     autoencoder_me = pre_train(num_epochs=epoch_pre, lr=lr_pre, save_step=5, autoencoder=autoencoder_me, train_loader=pre_trainloader, model_path=model_path)
    ...
    tgt_train = MyDataset(target_dt, target_prop)

    src_loader = DataLoader(src_data, batch_size=batch_size, shuffle=True)
    tgt_loader = DataLoader(tgt_train, batch_size=batch_size, shuffle=False)
    ...
    src_dict = {}
    src_dict.update({"train": src_loader})

    ## accuracy
    data = torch.tensor(np.array(source_dt)).to(device).to(torch.float32)
    autoencoder_me.eval()
    autoencoder_me.state = "test"

    with torch.no_grad():
        x_recon, z1, z, sigmatrix = autoencoder_me(data)
        src_result = z.cpu().detach().numpy()
        src_result_norm = refraction(src_result)
        ccc = CCCscore(src_result_norm, source_prop)
        # print(src_result_norm)
        print(ccc)

    data = torch.tensor(target_dt).to(device).to(torch.float32)
    autoencoder_me.eval()
    autoencoder_me.state = "test"

    with torch.no_grad():
        x_recon, z1, z, sigmatrix = autoencoder_me(data)
        tgt_result = z.cpu().detach().numpy()
        tgt_result_norm = refraction(tgt_result)
        ccc = CCCscore(tgt_result_norm, target_prop)
        # print(tgt_result_norm)
        print(ccc)
    ...
    autoencoder_me, discriminator_me = train_adda1(num_epochs=epoch_fine, lr=lr_train, save_step=5,
                                                  autoencoder=autoencoder_me, discriminator=discriminator_me,
                                                  src_data_loaders=src_dict,
    ...
    return autoencoder_me

def test_udapt(train_x, train_y, test_x, test_y, seed=0, run=1, iters_pre =200, iters_fine=200, lr_pre = 1e-5, lr_train = 1e-4, model_path='D:/Program/Pycharm/PyCharmProjects/Exp4/model', data_name='pancreas', run_now='baron'):
    source_dt, pre_dt, source_prop, pre_prop = trainTestSplit(train_x, train_y, test_x.shape[0], seed=0)
    batch_size = 128

    # # pseudo-bulk
    # autoencoder_me = train_model(pre_dt, pre_prop, source_dt, source_prop, test_x, test_y, batch_size=batch_size,
    #                              seed=0, run=run, epoch_pre=int(iters_pre / (len(pre_dt) / batch_size)),
    #                              epoch_fine=int(iters_fine / (len(pre_dt) / batch_size)), lr_pre=lr_pre,
    #                              lr_train=lr_train, model_path=model_path, data_name=data_name, run_now=run_now)
    ...
    print(epoch_fine)
    autoencoder_me = train_model(pre_dt, pre_prop, source_dt, source_prop, test_x, test_y, batch_size=batch_size,
                                 seed=0, run=run, epoch_pre=epoch_pre,
                                 epoch_fine=epoch_fine, lr_pre=lr_pre,
                                 lr_train=lr_train, model_path=model_path, data_name=data_name, run_now=run_now)
    ...
    with torch.no_grad():
        ...
        ccc = CCCscore(target_result, test_y)
    ...
    # target_result_df.columns = target_dt.columns
    #
    # target_sigmatrix_df = pd.DataFrame(target_sigmatrix)
    # target_sigmatrix_df.columns = target_dt.columns
    # target_sigmatrix_df.index = target_dt.index